 ...
 * @description 计算训练集的先验概率、条件概率(setup中进行)，并通过MapReduce任务计算测试集的每个文档分成每一类的概率
 */
public class GetNaiveBayesResult extends Configured implements Tool {
    public static final Logger log = LoggerFactory.getLogger(GetNaiveBayesResult.class);
    ...
    private static Map<String, Double> docTypePriorProbabilityMap = new HashMap<>(); // 每个文档 Ci 的先验概率 P(Ci)

    private static Map<String, Double> wordConditionalProbabilityMap = new HashMap<>(); // 每个单词 Wi 的条件概率 P(Wi|Ci)

    private static Map<String, String> docPredictResultMap = new HashMap<>(); // 每个文档的预测结果

    private static final Pattern ENGLISH_WORD_REGEX = Pattern.compile("^[A-Za-z]{2,}$"); // 单词的正则表达式


    static class GetNaiveBayesResultMapper extends Mapper<Text, BytesWritable, Text, Text> {
        // 测试单词的条件概率
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                reader = new SequenceFile.Reader(configuration, option);
            ...
            // 取出 sequence_file 中存储的类别中每个单词出现的次数，存储到 Map 中，形式为：CANA@hello 13
            try {
    ...
    static class GetNaiveBayesResultReducer extends Reducer<Text, Text, Text, Text> {
        // 测试集中文档被分为Ci类的概率
...
//            System.out.println(key.toString() + " 预测分类为： " + forecastDocType + " ，预测概率为：" + maxProbability);
    ...
    public int run(String[] strings) throws Exception {
        ...
        Job job = Job.getInstance(configuration, "GetNaiveBayesResultJob");

        job.setJarByClass(GetNaiveBayesResult.class);
        job.setMapperClass(GetNaiveBayesResultMapper.class);
        job.setCombinerClass(GetNaiveBayesResultReducer.class);
        job.setReducerClass(GetNaiveBayesResultReducer.class);
    ...
    public static void main(String[] args) throws Exception {
        int run = ToolRunner.run(new Configuration(), new GetNaiveBayesResult(), args);
        System.exit(run);
    }
}
